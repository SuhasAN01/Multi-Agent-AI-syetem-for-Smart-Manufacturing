...
export const generateWorkOrder = tool(
  async (params) => {
    ...
    // Idempotency check: Don't generate work order if alert_id is provided and already exists.
    if (rest.alert_id) {
      const existing = await db.collection("workorders").findOne({ alert_id: rest.alert_id });
      if (existing) {
        return JSON.stringify({ 
          message: "A work order for this alert ID already exists. Skipping duplicate creation.",
          workorder_id: existing._id 
        });
      }
    }
  ...
  },
  {
    ...
    schema: {
      ...
      properties: {
        ...
        alert_id: {
          type: "string",
          description: "ID of the alert/incident driving this work order.",
        },
        machine_id: {
          type: "string",
