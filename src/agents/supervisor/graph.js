...
import { digitalTwinAgent } from "../digitalTwin/graph.js";
...
const supervisorPrompt = ChatPromptTemplate.fromMessages([
  [
    ...
    You have four agents: digitalTwin, failure, workorder, and planning. Each agent should be called only once in a workflow.
    Given the conversation below, use the 'route_agent' tool to decide who should act next. The output must be one of: digitalTwin, failure, workorder, planning, __end__.
    1. Use 'digitalTwin' first when you receive an alert to simulate degradation and cascade risk.
    2. Use 'failure' after digitalTwin when an alert is not suppressed, to create an incident report.
    2. Use 'workorder' to create a workorder from an incident report.
    3. Use 'planning' to schedule a workorder.
    The end to end flow when receiving an alert is: digitalTwin -> failure -> workorder -> planning.
    If digitalTwin marks alert as suppressed, return '__end__'.
    You can answer general questions yourself, but if you receive an alert, you must route to this workflow.
...
function stateHasAlert(messages = []) {
  return messages.some(
    (msg) =>
      typeof msg?.content === "string" &&
      msg.content.includes("New alert received")
  );
}

export async function callModel(state) {
  // Deterministic workflow for alert orchestration.
  if (state.alertSuppressed) {
    return {
      next: "__end__",
  ...
  if (!state.lastAgent && stateHasAlert(state.messages)) {
    return { next: "digitalTwin", messages: state.messages || [], lastAgent: null };
  }
  if (state.lastAgent === "digitalTwin") {
    return { next: "failure", messages: state.messages || [], lastAgent: "digitalTwin" };
  }
  if (state.lastAgent === "failure") {
    return { next: "workorder", messages: state.messages || [], lastAgent: "failure" };
  }
  if (state.lastAgent === "workorder") {
    return { next: "planning", messages: state.messages || [], lastAgent: "workorder" };
  }
  if (state.lastAgent === "planning") {
    return { next: "__end__", messages: state.messages || [], lastAgent: null };
  }
...
export function shouldContinue(state) {
  ...
  if (next === "digitalTwin") return "digitalTwin";
...
export function agentNode(agentGraph, name) {
  return async (state) => {
    ...
    if (name === "failure") {
      nextState.crossMachinePatternsFound =
        result.crossMachinePatternsFound || state.crossMachinePatternsFound || [];
    }
    return nextState;
  };
}
...
export function createAgentGraph(client, dbName) {
  ...
  const builder = new StateGraph(StateAnnotation)
    ...
    .addNode("digitalTwin", digitalTwinAgent)
    ...
    .addEdge("digitalTwin", "supervisor")
