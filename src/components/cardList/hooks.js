  ...
  const getCardConfig = useCallback(
    (item, type) => {
      ...
      switch (type) {
        ...
        case "incident-reports":
          return {
            title: `${item.error_name || item.Err_name || item.alertId || "Incident Report"}${
              (item.machine_id || item.alertId) ? ` — Machine ${item.machine_id || item.alertId}` : ""
            }`,
            flagText: formatTimestamp(item.ts || item.created_at),
            description: item.report
              ? item.report.substring(0, 120) + (item.report.length > 120 ? "..." : "")
              : "",
            icon,
            titleColor,
            descColor,
