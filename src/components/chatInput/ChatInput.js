...
export default function ChatInput({
  ...
  if (!loadingAgents && (!agentOptions || agentOptions.length === 0)) {
    return (
      <div className="flex items-center justify-center h-full w-full p-4 text-gray-500">
        No agents available
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full w-full max-w-full">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center bg-white relative overflow-hidden m-5">
          ...
          {imageUrl && (
            <div className="relative w-[80%] h-[80%] mx-auto p-5">
              <Image
                src={imageUrl}
                alt="Agent Graph"
                fill
                className="object-contain"
                unoptimized // Bypasses Nextjs image optimization crashing on dynamically generated blob/base64 strings from Mermaid
              />
            </div>
          )}
        </div>
